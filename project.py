...
#frame
def detect(frame):
    bounding_box_cordinates, weights =  HOGCV.detectMultiScale(frame, winStride = (2, 2), padding = (8, 8), scale = 2.0)
    ...
    #x,y coordinates-----font------font size-------color------thickness
    cv2.putText(frame, f'Total Persons : {person-1}', (20,50), cv2.FONT_HERSHEY_DUPLEX, 0.5, (255,0,0), 1)
    cv2.imshow('output', frame)
    #Display the resulting image
    '''
    maximum=int(input("Enter the maximum count:"))
    server=smtplib.SMTP('smtp.gmail.com',587)
    server.starttls()
    if person-1>=maximum:
        server.login('projectbatch12mail@gmail.com','miniprojectbatch12')
        s='WARNING\nThe number of people detected in the area: '+str(person-1)+'\nThe count exceeded the given limit.\nTake necessary actions'
        server.sendmail('projectbatch12mail@gmail.com','renva2742002@gmail.com',s)
        print("sent")
    else:
        server.login('projectbatch12mail@gmail.com','miniprojectbatch12')
        server.sendmail('projectbatch12mail@gmail.com','renva2742002@gmail.com','At present the people count is under control.\nThe zone is safe.')
        print("sent")
'''
    return frame
...
#{'video': None, 'image': '1_233.jpg','output': None}
def argsParser():
...
if __name__ == "__main__":
    ...
    HOGCV.setSVMDetector(cv2.HOGDescriptor_getDefaultPeopleDetector())   #sets the Support Vector Machine detector to be the default human detector
